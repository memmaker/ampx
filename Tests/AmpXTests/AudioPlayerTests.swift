...
final class AudioPlayerTests: XCTestCase {
    ...
    func testTrackFinishedFiresOnlyAfterAudioHasPlayed() throws {
        ...
        // Poll rather than wait on an expectation: requirePlaybackStarted waits on all pending expectations
        let deadline = Date().addingTimeInterval(5.0)
        while finishedAt.value == nil, Date() < deadline {
            self.waitBriefly(0.05)
        }

        let elapsed = try XCTUnwrap(finishedAt.value, "Track should finish").timeIntervalSince(startedAt)
